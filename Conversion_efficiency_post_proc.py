...
class Conversion_efficiency(object):
    ...
    def __init__(self, freq_band, possition, filename=None, filepath='',
                 filename2='CE', filepath2='output_final/'):
        self.L = 18
        self.n = 1.444

        self.variables = ('P_p', 'P_s', 'f_p', 'f_s', 'l_p',
                          'l_s,' 'P_out', 'P_bef', 'CE', 'rounds')

        self.spec, self.fv, self.t, self.P0_p, self.P0_s, self.f_p,\
        self.f_s, self.P_bef, self.ro, U_large, tt,\
        self.u_large = self.load_spectrum('0', filename, filepath)
        self.P_max = np.max(w2dbm(self.spec))

        self.spec, self.fv, self.t, self.P0_p, self.P0_s, self.f_p,\
        self.f_s, self.P_bef, self.ro, U_large, tt, self.u_large =\
        ...
        try:
            ...
            fv_id_c = 0
        start_c, end_c = self.fv[fv_id_c] - \
            freq_band, self.fv[fv_id_c] + freq_band
        for i in U_large:
        ...
        read_write_CE_table(filename2, var=None, rin=self.rin, P_p=self.P0_p,
                            P_s=self.P0_s, f_p=self.f_p, f_s=self.f_s,
                            P_out=self.P_out, P_bef=self.P_bef, CE=self.CE,
                            var2='CE', std=self.std, file_path=filepath2)
    ...
    def calc_P_out(self, start, end):
        ...
        E_out = simps(self.spec[i:j]*(self.tt[1] -
                                      self.tt[0])**2, self.fv[i:j])
    ...
    def P_out_round(self, filepath, filesave):
        ...
        _data = {'pump_power': self.P0_p,
                 'pump_wavelength': self.l_p, 'out_wave': self.lam_wanted}
    ...
    def P_out_round_casc(self, filepath, filesave):
        ...
        _data = {'pump_power': self.P0_p,
                 'pump_wavelength': self.l_p, 'out_wave': self.lam_wanted}
        with open(filepath+'power_per_round_casc' +
                  filesave+'.pickle', 'wb') as f:
            pl.dump(fig, f)
        plt.clf()
        plt.close('all')
...
def read_write_CE_table(filename, var=None, rin=None, P_p=None,
                        P_s=None, f_p=None, f_s=None, P_out=None,
                        P_bef=None, CE=None, var2='CE', std=None,
                        file_path=''):
    ...
    a = pd.DataFrame(A, index=['P_p', 'P_s', 'f_p', 'f_s',
                               'l_s', 'l_p', 'P_out', 'P_bef', 'CE', 'rin']).T
    try:
...
def plot_rin(var, var2='rin', filename='CE',
             filepath='output_final/', filesave=None):
    var_val, CE, std = read_write_CE_table(
        filename, var, var2=var2, file_path=filepath)
...
def plot_CE(var, var2='CE', filename='CE',
            filepath='output_final/', filesave=None):
    var_val, CE, std = read_write_CE_table(
        filename, var, var2=var2, file_path=filepath)
    std = std[var2].as_matrix()
...
def contor_plot(CE, fmin=None, fmax=None,  rounds=None, filename=None):
    ...
    data = (CE.ro, CE.fv, z)
    _data = {'pump_power': CE.P0_p,
             'pump_wavelength': l_p, 'out_wave': CE.lam_wanted}
...
def contor_plot_anim(CE, iii, fmin=None,
                     fmax=None,  rounds=None, filename=None):
    if not(fmin):
        fmin = CE.fv[CE.fv_id] - CE.freq_band
    if not(fmax):
        fmax = CE.fv[CE.fv_id] + CE.freq_band
    ...
    al = ax.contourf(x, y, z, np.arange(-60, 2, 2),
                     extend='min', cmap=plt.cm.plasma)
    al2 = ax2.contourf(x, y, z, np.arange(-60, 2, 2),
                       extend='min', cmap=plt.cm.plasma)
...
def contor_plot_anim_single(CE, iii, fmin=None, fmax=None,
                            rounds=None, filename=None):
    if not(fmin):
        fmin = CE.fv[CE.fv_id] - CE.freq_band
    if not(fmax):
        fmax = CE.fv[CE.fv_id] + CE.freq_band
    ...
    al2 = ax.contourf(x, y, z, np.arange(-60, 2, 2),
                      extend='min', cmap=plt.cm.plasma)
    plt.ylim(285.5, 286.5)
    # plt.xlim(0,200)
    plt.ylabel(r'$f(THz)$')
...
def final_1D_spec(ii, specs, filename):
    ...
    ax1.plot(specs.fv, specs.spec_s, label=r'$\lambda_p$=' +
             str(specs.l_p)+r', $\lambda_s $='+str(specs.l_s))
    ...
    # plt.ylim(260,320)
    ax1.legend()
    plt.savefig(filename+'spectrum_fopo_final' +
                str(ii)+'.png', bbox_inches='tight')
    # plt.show()
...
inside_dirs = [[f for f in listdir(data_dump + '/'+out_dir)]
               for out_dir in outside_dirs]
...
for pos in ('4', '2'):
    ...
    for ii in outside_vec:
        ...
        os.system('mkdir output_final/'+str(ii)+'/pos'+pos+'/ ;'
                  ...
                  str(ii)+'/pos'+pos+'/casc_powers;'
                  + 'mkdir output_final/'+str(ii)+'/pos'+pos+'/final_specs;')

        for i in inside_vec[int(ii)]:
            print(ii, i)
            CE = Conversion_efficiency(
                2, possition=pos, filename='data_large', filepath=which +
                '/output'+str(i)+'/data/')

            # np.min(CE.fv),np.max(CE.fv),None
            fmin, fmax, rounds = 310, 330, 2000
            ...
            if animators:
                ...
                os.system('mkdir animators'+str(i)+'/contor animators' +
                          str(i)+'/power animators'+str(i)+'/contor_single')
                # sys.exit()
                for iii in spots:
                    contor_plot_anim(
                        CE, iii, fmin, fmax, rounds, filename='animators'
                        + str(i) + '/contor/'+str(iii))
                    contor_plot_anim_single(
                        CE, iii, fmin, fmax, rounds, filename='animators'
                        + str(i) + '/contor_single/'+str(iii))
                    P_out_round_anim(
                        CE, iii, filesave='animators' +
                        str(i)+'/power/'+str(iii))
                    gc.collect()
                giff_it_up(i, spots, 30)

            contor_plot(CE, fmin, fmax, rounds, filename='output_final/' +
                        str(ii)+'/pos'+pos+'/spectra/'+str(ii)+'_'+str(i))
            #contor_plot_time(CE, rounds = None,filename = 'output_final/'+str(ii)+'/pos'+pos+'/'+'time_'+str(ii)+'_'+str(i))
            CE.P_out_round(filepath='output_final/'+str(ii) +
                           '/pos'+pos+'/powers/', filesave=str(ii)+'_'+str(i))
            CE.P_out_round_casc(filepath='output_final/'+str(ii) +
                                '/pos'+pos+'/casc_powers/',
                                filesave=str(ii)+'_'+str(i))
            final_1D_spec(i, CE, filename='output_final/' +
        ...
        for var1, var2 in (('P_p', 'P_out'), ('P_p', 'CE')):
            plot_CE(var1, var2, filesave='output_final/' +
                    str(ii)+'/pos'+pos+'/many/'+var2+str(ii))
        plot_rin('P_p', 'rin', filesave='output_final/' +
                 str(ii)+'/pos'+pos+'/many''/rin_'+str(ii))
