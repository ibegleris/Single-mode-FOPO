...
def plotter_dbm(index, nm, sim_wind, u, U, P0_p, P0_s, f_p,
                f_s, which, ro, pump_wave='', filename=None,
                title=None, im=0, plots=True):
    ...
    if plots == True:
        ...
        if filename == None:
            ...
            plt.savefig('output'+pump_wave+'/output'+str(index) +
                        '/figures/wavelength/'+filename, bbox_inched='tight')
        ...
        if filename == None:
            ...
            plt.savefig('output'+pump_wave+'/output'+str(index) +
                        '/figures/freequency/'+filename, bbox_inched='tight')
        plt.close(fig)
        ...
        if filename == None:
            ...
            plt.savefig('output'+pump_wave+'/output' +
    ...
    if filename is not(None):
        ...
        try:
            ...
            save_variables('data_large', layer,
                           filepath='output'+pump_wave +
                           '/output'+str(index)+'/data/',
                           U=U[:, which], t=sim_wind.t, u=u[:, which],
                           fv=sim_wind.fv, lv=sim_wind.lv,
                           which=which, nm=nm, P0_p=P0_p,
                           P0_s=P0_s, f_p=f_p, f_s=f_s, ro=ro)
        except RuntimeError:
            ...
            save_variables('data_large', layer,
                           filepath='output'+pump_wave +
                           '/output'+str(index)+'/data/',
                           U=U[:, which], t=sim_wind.t,
                           u=u[:, which], fv=sim_wind.fv,
                           lv=sim_wind.lv, which=which, nm=nm,
                           P0_p=P0_p, P0_s=P0_s, f_p=f_p, f_s=f_s, ro=ro)
            pass
...
def animator_pdf_maker(rounds, pump_index):
    ...
    for sp in space:
        ...
        for i in range(4):
            os.system(
                'convert -delay 30 '+file_loc+str(i)+'.pdf '
                + file_loc+str(i)+'.mp4')
